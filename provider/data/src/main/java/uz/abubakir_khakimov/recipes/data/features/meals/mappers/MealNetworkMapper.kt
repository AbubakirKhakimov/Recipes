package uz.abubakir_khakimov.recipes.data.features.meals.mappers
...
internal class MealNetworkMapper: EntityMapper<MealNetworkEntity, MealDataEntity> {

    override fun mapTo(entity: MealNetworkEntity): MealDataEntity =
        MealDataEntity(
    ...
    override fun mapToList(entityList: List<MealNetworkEntity>): List<MealDataEntity> =
        entityList.map { mapTo(entity = it) }

    override fun mapToResultList(
        entityResultList: Result<List<MealNetworkEntity>>
    ): Result<List<MealDataEntity>> = entityResultList.map { attendanceList ->
        mapToList(entityList = attendanceList)
    }

    override fun mapToResult(entityResult: Result<MealNetworkEntity>): Result<MealDataEntity> =
