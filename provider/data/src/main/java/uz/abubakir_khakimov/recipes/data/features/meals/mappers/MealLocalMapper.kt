...
internal class MealLocalMapper: EntityMapper<MealLocalEntity, MealDataEntity> {

    override fun mapTo(entity: MealLocalEntity): MealDataEntity =
        MealDataEntity(
    ...
    override fun mapToList(entityList: List<MealLocalEntity>): List<MealDataEntity> =
        entityList.map { mapTo(it) }

    override fun mapToResultList(
        entityResultList: Result<List<MealLocalEntity>>
    ): Result<List<MealDataEntity>> = entityResultList.map { mapToList(it) }
}