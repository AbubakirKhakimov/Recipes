...
class MealToDataMapper: EntityMapper<Meal, MealDataEntity> {

    override fun mapTo(entity: Meal): MealDataEntity =
        MealDataEntity(
    ...
    override fun mapToList(entityList: List<Meal>): List<MealDataEntity> =
        entityList.map { mapTo(entity = it) }

    override fun mapToResultList(
        entityResultList: Result<List<Meal>>
    ): Result<List<MealDataEntity>> = entityResultList.map { attendanceList ->
        mapToList(entityList = attendanceList)
    }

    override fun mapToResult(entityResult: Result<Meal>): Result<MealDataEntity> =
